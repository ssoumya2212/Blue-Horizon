...
function connectDatabaseEmulator(db2, host, port, options = {}) {
  ...
  if (db2._instanceStarted) {
    if (hostAndPort === db2._repoInternal.repoInfo_.host && deepEqual(options, repo.repoInfo_.emulatorOptions)) {
      return;
    }
...
function startSimulator() {
  ...
  setInterval(() => {
    positions = positions.map(
      (b) => b.status === "Idle" ? b : {
        ...b,
        lat: b.lat + (Math.random() - 0.5) * 8e-4,
        lng: b.lng + (Math.random() - 0.5) * 8e-4
      }
    );
    emit();
  }, 2e3);
...
function startFirebase() {
  ...
  if (!db2) return;
  onValue(ref(db2, "buses"), (snap) => {
    const val = snap.val();
    if (!val) return;
    const next = Array.isArray(val) ? val.filter(Boolean) : Object.values(val);
    positions = next;
    emit();
  });
}
...
function useFleetPositions() {
  const [state, setState] = reactExports.useState(positions);
...
const trackingSource = firebaseEnabled ? "firebase" : "simulated";
export {
  trackingSource as t,
  useFleetPositions as u
};
