...
export const adminCreateUser = createServerFn({ method: "POST" })
  .inputValidator(
    z.object({
      token: z.string(),
      email: z.string().email().endsWith("@gmail.com", { message: "Only @gmail.com email addresses are allowed for security reasons" }),
...
export const adminUpdateUser = createServerFn({ method: "POST" })
  .inputValidator(
    z.object({
      token: z.string(),
      id: z.string(),
      email: z.string().email().endsWith("@gmail.com", { message: "Only @gmail.com email addresses are allowed for security reasons" }).optional(),
    ...
    async ({
      ...
      try {
        ...
        const { data: existingStudent } = await supabaseAdmin
          .from("students")
          .select("id")
          .eq("student_roll_no", registerNo)
          .maybeSingle();

        const studentQuery = existingStudent?.id
          ? supabaseAdmin
              .from("students")
              .update(dbStudent)
              .eq("id", existingStudent.id)
              .select()
          : supabaseAdmin.from("students").insert([dbStudent]).select();
...
export const adminDeleteNotification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: { token, id } }) => {
    try {
      ...
      console.error("adminDeleteNotification error:", error);
      return { success: false, error: error.message };
    }
  });

export const adminClearNotifications = createServerFn({ method: "POST" })
  .validator(z.object({ token: z.string() }))
  .handler(async ({ data: { token } }) => {
    try {
      await verifyAuth(token, true);
      const supabaseAdmin = getSupabaseAdmin();
      const { error } = await supabaseAdmin.from("notifications").delete().not("id", "is", null);
      if (error) throw error;
      return { success: true };
    } catch (error: any) {
      console.error("adminClearNotifications error:", error);
      return { success: false, error: error.message };
    }
  });

export const adminEditNotification = createServerFn({ method: "POST" })
  .validator(z.object({ token: z.string(), id: z.string(), message: z.string() }))
  .handler(async ({ data: { token, id, message } }) => {
    try {
      await verifyAuth(token, true);
      const supabaseAdmin = getSupabaseAdmin();
      const { error } = await supabaseAdmin.from("notifications").update({ message }).eq("id", id);
      if (error) throw error;
      return { success: true };
    } catch (error: any) {
      console.error("adminEditNotification error:", error);
      return { success: false, error: error.message };
    }
  });
