...
function DriverDashboard() {
  ...
  const handleStartTrip = async () => {
    ...
    try {
      ...
      await addNotification(
        `Trip Started`,
        `Bus ${assignedBus.id} has started its route. Live tracking is now active!`,
        "trip_start",
        "parent"
      );
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden p-0 border-0 shadow-[var(--shadow-card)]">
        <div className="border-b bg-muted/30 px-5 py-3">
          <h2 className="font-semibold">Live Route Map</h2>
        </div>
        <div className="h-[300px] w-full">
          <FleetMap buses={fleet} highlightId={assignedBus?.id || ""} showAccessibleList={false} showAllRoutes={true} />
        </div>
      </Card>
