...
function ParentDashboard() {
  ...
  useEffect(() => {
    async function loadRoutePath() {
      if (route?.description) {
        const path = await buildRoutePathAsync(route.description);
        setRoutePath(path);
      }
    }
    loadRoutePath();
  ...
  const dynamicStops = routePath?.stopNames.map((stopName: string, index: number) => ({
    name: stopName,
    time: index === 0 ? "Route Start" : index === routePath.stopNames.length - 1 ? "Route End" : `Stop ${index + 1}`,
    dist: index === 0 ? "Starting Point" : index === routePath.stopNames.length - 1 ? "Destination" : `Intermediate Stop ${index + 1}`,
    state:
      index === 0 && (student?.status === "pending" || student?.status === "picked")
        ? "Current"
        : index === routePath.stopNames.length - 1 && student?.status === "dropped"
          ? "Current"
          : index === 1
            ? "Next"
            : "",
  ...
  const handleSavePickup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!student || !editingPickupAddress) return;
    setSavingPickup(true);
    const { error } = await supabase
      .from("students")
      .update({ pickup_address: editingPickupAddress })
      .eq("id", student.id);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Dialog open={openEditPickup} onOpenChange={setOpenEditPickup}>
              <DialogTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => setEditingPickupAddress(student.pickup_address || "")}
                  className="bg-white/20 hover:bg-white/30 text-white border-white/40 h-6 text-xs px-2"
                >
                  <SettingsIcon className="mr-1 h-3 w-3" /> Edit Pickup
                </Button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-[425px]">
                ...
                <form onSubmit={handleSavePickup} className="space-y-4 py-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Pickup Point</label>
                    <LocationPicker
                      value={editingPickupAddress}
                      onChange={setEditingPickupAddress}
                      placeholder="Enter pickup address or pick on map"
                      required
                    />
                  </div>
                  <DialogFooter>
                    <Button type="button" variant="outline" onClick={() => setOpenEditPickup(false)} disabled={savingPickup}>Cancel</Button>
                    <Button type="submit" disabled={savingPickup}>{savingPickup ? "Saving..." : "Save Location"}</Button>
                  </DialogFooter>
                </form>
              </DialogContent>
            </Dialog>
      ...
      <Card className="overflow-hidden border p-4 shadow-[var(--shadow-card)]">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold">Live Bus Route</h2>
            <p className="text-sm text-muted-foreground">
              Track your child&apos;s bus and view the full route stop by stop.
            </p>
          </div>
          <Badge variant="outline">
            {bus?.id ? `Bus ${bus.id}` : "Bus Unassigned"}
          </Badge>
        </div>
        <div className="h-[360px] overflow-hidden rounded-xl border">
          <FleetMap
            ...
            routeStops={routePath?.coordinates || []}
          />
        </div>
      </Card>
