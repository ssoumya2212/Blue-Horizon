...
export async function fetchParentsList(): Promise<Parent[]> {
  try {
    ...
    const mapped = (profiles || []).map((p) => ({
      ...
      password_changed: Boolean(p.password_changed),
...
export async function updateParentStatus(id: string, disable: boolean) {
  const token = (await supabase.auth.getSession()).data.session?.access_token || "";
  const res = await adminDisableUser({ data: { token, id, disable } });

  if (!res.success) {
    toast.error(`Failed to ${disable ? "disable" : "enable"} parent: ${res.error}`);
    return false;
  }

  toast.success(`Parent ${disable ? "disabled" : "enabled"} successfully`);
  fetchParentsList();
  return true;
}
