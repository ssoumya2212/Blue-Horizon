...
import {
  adminDeleteNotification as serverDeleteNotification,
  adminClearNotifications as serverClearNotifications,
  adminEditNotification as serverEditNotification,
...
export async function deleteNotification(id: string) {
  ...
  const res = await serverDeleteNotification({ data: { token, id } });
  if (!res.success) {
    toast.error(`Failed to delete notification: ${res.error}`);
    return false;
  }
  return true;
}
...
export async function clearAllNotifications() {
  const token = (await supabase.auth.getSession()).data.session?.access_token || "";
  const res = await serverClearNotifications({ data: { token } });
  if (!res.success) {
    toast.error(`Failed to clear notifications: ${res.error}`);
    return false;
  }
  return true;
}

export async function editNotification(id: string, message: string) {
  const token = (await supabase.auth.getSession()).data.session?.access_token || "";
  const res = await serverEditNotification({ data: { token, id, message } });
  if (!res.success) {
    toast.error(`Failed to edit notification: ${res.error}`);
    return false;
  }
  return true;
}
