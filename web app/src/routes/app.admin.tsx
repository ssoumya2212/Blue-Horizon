...
function AdminDashboard() {
  ...
  const loadDb = async () => {
    ...
    const { count: parentCount } = await supabase
      ...
      .select("*", { count: "exact", head: true })
      .eq("role", "parent");
    const { count: activeParentCount } = await supabase
      .from("profiles")
      .select("*", { count: "exact", head: true })
      .eq("role", "parent")
    ...
    setStatsData({
      ...
      activeParents: activeParentCount || 0,
  ...
  const dynamicStats = [
    ...
    },
    {
      ...
      trend: `${statsData.activeParents} active accounts`,
      icon: Users,
      tone: "primary",
    },
    {
    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="w-full overflow-hidden rounded-xl h-[400px]">
          <FleetMap buses={fleet} showAccessibleList={false} showAllRoutes={true} />
        </div>
      </Card>
    ...
    try {
      if (kind === "route") {
        ...
        if (!licenseFile || licenseFile.size === 0) {
          throw new Error("Licence Document is required.");
        }
        ...
        const d = await addDriverService({
          ...
          bus_id: data.get("bus_id") ? String(data.get("bus_id")) : undefined,
            ...
            <div className="grid gap-4 py-4">
              {kind === "driver" ? (
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="licenseExpiry">Licence Expiry Date</Label>
                    <Input
                      id="licenseExpiry"
                      name="licenseExpiry"
                      type="date"
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="medical_certificate">Medical Certificate upload</Label>
                    <Input
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="license_document">Licence Document upload</Label>
                    <Input
                      id="license_document"
